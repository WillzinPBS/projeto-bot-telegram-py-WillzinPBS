...
with open("funcionarios.json", encoding='utf-8') as funcionarios:
    funcionarios = json.load(funcionarios)

horaAtual = datetime.now().strftime('%H:%M')

#importo as taxas dos sites
...
class TelegramBot:
    def __init__(self):
        token = '2112687757:AAGmdjG0zo8XhSNl_y2O6YbfqZyWkUIGwY8'
        ...
        while True:
            ...
            if mensagens:
                for mensagem in mensagens:
                    ...
                    if resposta == 'teste foto':
                        for f in funcionarios:
                            if(f['user_id'] == chat_id):
                                funcionario = f['nome']

                        self.enviarImagem('imagens\{}.png'.format(funcionario), chat_id)
    ...
    def enviarImagem(self, file_path, chat_id):
        token = '2112687757:AAGmdjG0zo8XhSNl_y2O6YbfqZyWkUIGwY8'
